...
**hermes** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `hermes` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
hermes ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
hermes ... --format json | cognis-connect emit --to splunk --url $URL --token $TOK   # Splunk HEC
hermes ... --format json | cognis-connect emit --to elastic --url $URL --token $TOK   # Elastic _bulk
hermes ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `hermes` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="hermes") for rec in records]   # records = your JSON output
