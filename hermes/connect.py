"""Native cognis-connect emit for hermes — forward findings to any platform.

Maps hermes's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    hermes ... --format json | hermes-emit --to stix
    hermes-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        ...
        out['title'] = out.get('content', '')
        out['severity'] = out.get('confidence', 'info')
        out['type'] = out.get('category', 'unknown')
